...
import PacmanTodo from '../components/PacmanTodo';
import HowTo from '../components/HowTo';
import RightHomeInfoPacman from '../components/RightHomeInfoPacman';
...
const q = query(collection(db, 'pacmantodos'), orderBy('timestamp', 'desc'));
...
// const InputPacmanMintAddress = styled.input`
//   min-width: 16rem;
//   font-size: 1rem;
//   padding: 0.6rem 1.5rem 0.6rem 1.5rem;
...
const InputBlinkyMintAddress = styled.input`
  min-width: 16rem;
  font-size: 1rem;
  padding: 0.6rem 1.5rem 0.6rem 1.5rem;
...
function Pacman() {
    const [pacmantodos, setPacmanTodos] = useState([]);

    const [inputGameName, setInputGameName] = useState('');
  
    // const [inputPacmanMintAddress, setInputPacmanMintAddress] = useState('');
  
    const [inputBlinkyMintAddress, setInputBlinkyMintAddress] = useState('');
    ...
    // const pacmannftPic = GetNFTData(inputPacmanMintAddress);
  
    const blinkynftPic = GetNFTData(inputBlinkyMintAddress);
    ...
    useEffect(() => {
      onSnapshot(q, (snapshot) => {
        setPacmanTodos(snapshot.docs.map(doc => ({
          id: doc.id,
          item: doc.data()
        })))
      })
    }, [inputGameName, inputBlinkyMintAddress]);
    ...
    const addPacmanTodo = (e) => {
      e.preventDefault();
      if ((inputGameName !== '')) {
        addDoc(collection(db, 'pacmantodos'), {
          gameName: inputGameName,
          // pacmanMintAddress: inputPacmanMintAddress,
          blinkyMintAddress: inputBlinkyMintAddress,
          ...
          // pacmanNFTPic: pacmannftPic,
          blinkyNFTPic: blinkynftPic,
          // invaderTwoNFTPic: invadertwonftPic,
          // invaderThreeNFTPic: invaderthreenftPic,
          // invaderUfoNFTPic: invaderufonftPic,
          timestamp: serverTimestamp()
        })
        // window.location.href = 'https://www.r3x.tech/#pacmangames'
        setInputGameName('')
        // setInputPacmanMintAddress('')
        setInputBlinkyMintAddress('')
      ...
      <div className="pacmancontainer">
          <div>
            ...
            <form style={{ border: '1px solid white' }}>
              ...
              {/* <div style={{ textAlign: "center" }}>
                <InputPacmanMintAddress type="text" placeholder='Pacman Mint Address' value={inputPacmanMintAddress} onBlur={(e) => e.target.placeholder = 'Pacman Mint Address'} onInput={(e) => e.target.color = 'white'} onFocus={(e) => e.target.placeholder = ''} onChange={e => setInputPacmanMintAddress(e.target.value)} >
                </InputPacmanMintAddress>
              </div> */}
  
              <div style={{ textAlign: "center" }}>
                <InputBlinkyMintAddress type="text" placeholder='Blinky Mint Address' value={inputBlinkyMintAddress} onBlur={(e) => e.target.placeholder = 'Blinky Mint Address'} onInput={(e) => e.target.color = 'white'} onFocus={(e) => e.target.placeholder = ''} onChange={e => setInputBlinkyMintAddress(e.target.value)} >
                </InputBlinkyMintAddress>
              </div>
              ...
              <div style={{ textAlign: "center", paddingTop: "1rem", paddingBottom: "1rem" }}>
                <ButtonTwo onClick={addPacmanTodo}>GENERATE GAME</ButtonTwo>
              </div>
            </form>
        ...
        <div style={{ margin: "0rem 0rem 6rem 0rem" }}>
          ...
          <div>
            {pacmantodos.map(item => <PacmanTodo key={item.id} arr={item} />)}
          </div>
        </div>
